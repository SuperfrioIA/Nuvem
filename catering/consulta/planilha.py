...
divergir. O aceite do V3.3 mede isso: somando todas as paginas da planilha, o
total tem que dar **exatamente** o que a Matriz agrega no mesmo recorte. Se as
duas discordarem, uma das duas esta mentindo.

## Estreita na tela, completa no arquivo

A tela mostra dia, unidade, cliente, guia, operacao, tipo de estoque e a
**lente escolhida** (na saida, as tres faixas dela). Mostrar as 16 medidas da
expedicao seria o "indo pro lado" que a Matriz evitou -- e a planilha e ainda
mais larga que ela.

O **download** leva a linha inteira (`download.py`). Essa e a divisao: a tela
responde "o que aconteceu nessa guia", o arquivo responde "me da tudo para eu
analisar".
...
na **chave natural**, que e unica por construcao (V3.0) -- e com isso a
paginacao e total.

## `guia` aparece aqui, e nao na Matriz

O contrato tira a guia da Matriz porque contagem distinta nao soma por linha
(`COUNT(DISTINCT)` num pivo daria numero errado). Na planilha ela e coluna de
uma linha, nao agregado -- entao entra.
"""
